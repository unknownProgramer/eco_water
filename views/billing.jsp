...
<%
    ...
    if (arr.size() > 0) {
        System.out.println("data size : " + arr.size());
    } else {
...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark"
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link text-white  active bg-gradient-info" href="../pages/billing.jsp">
                    <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
                        <i class="material-icons opacity-10">opacity</i>
                    </div>
        ...
        <div class="card card-body mx-3 mx-md-4 mt-n6">
            ...
            <div class="bg-gradient-light shadow-card border-radius-lg pt-4 pb-3">
                ...
                <div align="center">
                    ...
                    <div align="center" border="10px"><input style="display: inline-block;" type='button'
                                                             onclick="month('minus')" value="◀"></input>
                        <div id='month' style="display: inline-block;"><%=month%>
                        </div>
                        월
                        <input style="display: inline-block;" type="button" onclick="month('plus')" value="▶"></input>
                    </div>
                    <br>
                </div>
                ...
                <div class="card z-index-2  ">
                    <div class="card-header p-0 position-relative t-4 pb-3 z-index-2 bg-transparent">
                        <div class="bg-gradient-white shadow-success border-radius-lg pt-4 pb-3">
                            <div class="chart">
                                <canvas id="chart-line-tasks" class="chart-canvas" height="212" width="352"
                                        style="display: block; box-sizing: border-box; height: 500px; width: 10000px;"></canvas>
                            </div>
                        </div>
                    </div>

                    <div class="card-body">
                        <h3 class="mb-0 font-italic"> 전일수량 및 평균수량 </h3>
            ...
            <div class="row align-items-center justify-content-lg-between">
                <div class="col-lg-6 mb-lg-0 mb-4">
                    <div class="copyright text-center text-sm text-muted text-lg-start">
                        ...
                        <a href="https://www.netflix.com" class="font-weight-bold" target="_blank">Project made by</a>
                        <Strong>김강훈,</Strong>
                        최낙현, 황인성, 최석재, 정세빈
                    </div>
                </div>
...
<script>
    ...
    $(function () {
        let chartInfo = {
            type: "line",
            data: {
                labels: [<%for(int i = arr.size()-1; i >= 0; i--){ System.out.println((arr.get(i).getDate().split("-")[1]+"/"+arr.get(i).getDate().split("-")[2].substring(0,2)));%>
                    <%=(arr.get(i).getDate().split("-")[1])%>+"/" +<%=(arr.get(i).getDate().split("-")[2].substring(0,2))%>,
                    <%}%>],
                datasets: [{
                    label: "당일수량", //차트 제목
                    tension: 0,
                    suggestedMin: 1000, // y축 최소값
                    suggestedMax: 10000, //y축 최대값
                    borderWidth: 0,
                    pointRadius: 5,
                    pointBackgroundColor: "rgba(255, 100, 0, 0.8)",
                    pointBorderColor: "transparent",
                    borderColor: "rgba(255,100,0,0.8)",
                    borderWidth: 3,  //경계선굵기
                    backgroundColor: "transparent",
                    fill: true, //line형태일때 선안쪽 채우는지? 안채우는지
                    data: [<%for(int i = arr.size()-1; i >= 0; i--){ System.out.println(arr.get(i).getStorage());%>
                        <%=arr.get(i).getStorage() %>,
                        <%}%>],
                    maxBarThickness: 20
                },
                    //   {
                    // label: ["123","test","123","12522552","1252552","12355"],
                    // datasets:[{
                    //     label: "test",
                    //     tension: 0,
                    //     borderWidth: 0,
                    //     pointRadius: 5,
                    //     pointBackgroundColor: "rgba(0,51,255,0.8)",
                    //     pointBorderColor: "transparent",
                    //     borderColor: "rgba(12,0,255,0.8)",
                    //     borderWidth: 3,  //경계선굵기
                    //     backgroundColor: "transparent",
                    //     fill: true, //line형태일때 선안쪽 채우는지? 안채우는지
                    //     data: [ 0,200,500,600,700,800],
                    //     maxBarThickness: 5,
        ...
        wChart = new Chart(ctx3, chartInfo);
    ...
    function month(type) {
        // 결과를 표시할 element
        console.log("click")
        const resultElement = document.getElementById('month');

        // 현재 화면에 표시된 값
        let month = resultElement.innerText;
        // 더하기/빼기
        if (type === 'plus') {
            if (month < 12) {
        ...
        $.ajax({
            url: "http://localhost:3307/getDailyWaterStorage",
            type: "get",
            data: {"month": month},
            dataTypes: "json",
            success: function (d) {
